...
function Model(opts) {
	...
	var createInstance = function (data, inst_opts, cb) {
		...
		var instance = new Instance(model, {
			...
			partitionKeysProperties: partitionKeysProperties
		});
	...
	model.createBatch = function (dataArray, options, callback) {
		...
		opts.driver.batchInsert(tableName, batchData, keyProperties, batchSize,
	...
	for (k in opts.properties) {
		...
		if(opts.partitionKeys.indexOf(k) !== -1) {
			prop.partitionKey = true;
			partitionKeysProperties.push(prop);
		}
