...
// patch to allow for driver adapters to be written as es6 classes and associations to still work.
exports.typeOf = function (obj) {
  const constructorType = obj.constructor.toString().match(/^(.+?) (Object|String|Number())?/);
  try {
    if (constructorType[2]) return constructorType[2].toLowerCase();
    return constructorType[1];
  } catch (e) {
    return typeof obj;
  }
}