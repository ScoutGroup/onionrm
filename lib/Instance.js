...
function Instance(Model, opts) {
	...
	var rememberKeys = function () {
		...
		for(i = 0; i < opts.updateKeys.length; i++) {
			propName = opts.updateKeys[i];
			opts.originalUpdateKeyValues[propName] = opts.data[propName];
		}
	...
	var savePersisted = function (saveOptions, data, cb) {
		...
		if (opts.changes.length === 0) {
			...
			for (i = 0; i < opts.updateKeys.length; i++) {
				propName = opts.updateKeys[i];
				conditions[propName] = opts.originalUpdateKeyValues[propName];
			}
