...
function Instance(Model, opts) {
	...
	var rememberKeys = function () {
		...
		for(i = 0; i < opts.updateKeysProperties.length; i++) {
			prop = opts.updateKeysProperties[i];
			opts.originalUpdateKeyValues[prop.name] = opts.data[prop.name];
		}
	...
	var savePersisted = function (saveOptions, data, cb) {
		...
		if (opts.changes.length === 0) {
			...
			for (i = 0; i < opts.updateKeysProperties.length; i++) {
				prop = opts.updateKeysProperties[i];
				conditions[prop.mapsTo] = opts.originalUpdateKeyValues[prop.name];
			}
