...
function Instance(Model, opts) {
	...
	opts.originalPartitionKeyValues = {};
	...
	var rememberKeys = function () {
		...
		for(i = 0; i < opts.partitionKeysProperties.length; i++) {
			prop = opts.partitionKeysProperties[i];
			opts.originalpartitionKeyValues[prop.name] = opts.data[prop.name];
		}
	...
	var savePersisted = function (saveOptions, data, cb) {
		...
		if (opts.changes.length === 0) {
			...
			for (i = 0; i < opts.partitionKeysProperties.length; i++) {
				prop = opts.partitionKeysProperties[i];
				conditions[prop.mapsTo] = opts.originalPartitionKeyValues[prop.name];
			}
